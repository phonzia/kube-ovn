...
func (c *ovnClient) CreateLogicalSwitchPort(lsName, lspName, ip, mac, podName, namespace string, portSecurity bool, securityGroups, vips string, liveMigration bool, enableDHCP bool, dhcpOptions *DHCPOptionsUUIDs) error {
	...
	if liveMigration {
		ports, err := c.ListNormalLogicalSwitchPorts(true, map[string]string{"ip": strings.ReplaceAll(ip, ",", "/")})
		if err != nil {
			klog.Errorf("ListNormalLogicalSwitchPorts failed: %v", err)
			return err
		}
		if len(ports) > 0 {
			isAddrConflict = true
...
// SetLogicalSwitchPortDHCPv4Options set logical switch port address
func (c *ovnClient) SetLogicalSwitchPortAddress(lspName, mac, ips string) error {
	lsp, err := c.GetLogicalSwitchPort(lspName, false)
	if err != nil {
		return fmt.Errorf("get logical switch port %s: %v", lspName, err)
	}
	...
	lsp.Addresses = []string{strings.Join(addresses, " ")} // addresses is the first element of addresses

	if err := c.UpdateLogicalSwitchPort(lsp, &lsp.Addresses); err != nil {
		return fmt.Errorf("set logical switch port %s addresses %v: %v", lspName, lsp.Addresses, err)
	}
